...
/**
 * A class to contain info on a building.
 * 
 * @author michael
 */
public class BuildingInfo {
	...
	/**
	 * Filling bar that shows how far the building is in the process of being
	 * robbed
	 */
	private FillingBar fillingBar;

	// can be accessed through the building field but why add a layer of
	// indirection when it won't change
	/**
	 * String containing the building type. Can access it through the building
	 * field
	 */
	...
	public BuildingInfo(Building bldg) {
		...
		this.frame = new Rectangle(bldg.position.getX(), bldg.position.getY(),
				bldg.width, BUILDING_INFO_HEIGHT);
		...
		fillingBar = new FillingBar(frame.getX(), frame.getY(),
				frame.getWidth());
	}

	/**
	 * Draw the building's info above it
	 * 
	 * @param x
	 * @param y
	 */
	public void draw(float x, float y) {
		...
		g.drawString(
				String.format("%d/%d", bldg.occupants.size()
						- bldg.occupantsOnVacation, bldg.occupants.size()),
				frame.getX(), frame.getY() - 25);

		g.drawString(String.format("$%d", bldg.money), frame.getX(),
				frame.getY() - 12);
